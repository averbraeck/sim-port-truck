 ...
 */
public class CoordinateTransformRdNewToWgs84 implements CoordinateTransform, Serializable
{
    /** */
     ...
     */
    public CoordinateTransformRdNewToWgs84(final double dx, final double dy)
    {
        this.dx = dx;
        this.dy = dy;
    }
    ...
    public final DoubleXY doubleTransform(final double x, final double y)
    {
        try
        {
            Point2D c = TransformWgs84DutchRdNew.toWgs84(x - this.dx, y - this.dy);
            return new DoubleXY(c.getX(), c.getY());
        }
    ...
    {
        return "CoordinateTransformRdNewToWgs84 [dx=" + this.dx + ", dy=" + this.dy + "]";
    }
}
